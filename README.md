...
```text
bilingual_node_names-0.5.2.zip
```

## 主な機能

- Geometry NodesとShader Nodesの主要ノードを `English / 日本語` で表示
- Blenderに登録された全Nodeサブクラスを、クラス名に依存せず自動検出
- Geometry／Shader両ツリーで実際に生成し、利用可能なツリーを自動判定
- 辞書の分類より実機生成結果を優先し、両ツリー対応ノードの検索漏れを防止
- 実行中に追加登録された外部アドオンの独自ノードも自動再探索
- Blender同梱のGeometry／Shader Node Groupアセットを自動検出
- Blender 5.2の`Cloth Dynamics (Experimental)`、`Collider`などの物理アセットを日英検索して追加
- Blender公式日本語辞書を利用した未登録ノード名の自動補完
- 標準Shift+Aメニューから英語名・日本語名で検索
- 起動直後や検索インデックス再構築中も、候補と追加ノードのID対応を固定
- 日本語の別名・用途語に対応した独自検索
- 通常表示と日英表示の切替
- 通常表示中も二言語検索を維持
- 新規ノードへの自動適用
...
実ノード生成、Operator、データ不変性、全Nodeサブクラスの探索完全性、実行時追加ノード、破損辞書、1,000ノード性能を含む厳格テスト:
...
探索完全性テストでは、Blender 4.5.10 LTSで523型・403生成可能組、Blender 5.2.0 LTSで570型・478生成可能組を照合し、登録結果との完全一致を確認しています。Blender 5.2同梱のNode Groupアセット38件も追加可能性を実検証しています。
